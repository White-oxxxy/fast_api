...
class UserService(IUserService):
    ...
    async def get_by_username(self, username: str) -> User:
        async with self.database.read_only_async_session as session:
            ...
            user_orm = await repo.get_by_username(username=username)
            return GetUserFromORM.execute(user_orm)

    async def create(self, user: UserInput) -> User:
        async with self.database.async_session as session:
            repo = self.__get_repo(session=session)
            user_create: UserCreate = UserInputToUserCreate.execute(user=user)
            user_orm = await repo.create(user=user_create)
            await repo.commit()
            return GetUserFromORM.execute(user_orm)
    ...
    async def get_or_create(self, user: UserInput) -> User:
        async with self.database.async_session as session:
            repo = self.__get_repo(session=session)
            user_orm = await repo.get_by_username(username=user.username)
            if not user_orm:
                user_create: UserCreate = UserInputToUserCreate.execute(user=user)
                user_orm = await repo.create(user=user_create)
                await repo.commit()
    ...
    async def _get_by_oid(self, required_oid: UUID) -> User | None:
        async with self.database.read_only_async_session as session:
            repo = self.__get_repo(session=session)
            user_orm = await repo.get_by_oid(required_oid=required_oid)
            if not user_orm:
                return None
            return GetUserFromORM.execute(user_orm)
